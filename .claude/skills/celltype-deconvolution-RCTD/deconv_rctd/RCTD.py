...
def LoadLikelihoodTable(ext_dir):
    with gzip.open(f'{ext_dir}/Q_mat_1_1.txt.gz','rt') as f:
        lines = f.readlines()
    with gzip.open(f'{ext_dir}/Q_mat_1_2.txt.gz','rt') as f:
        lines += f.readlines()
    ...
    with gzip.open(f'{ext_dir}/Q_mat_2_1.txt.gz','rt') as f:
        lines2 = f.readlines()
    with gzip.open(f'{ext_dir}/Q_mat_2_2.txt.gz','rt') as f:
        lines2 += f.readlines()
    ...
    with open(f'{ext_dir}/X_vals.txt') as f:
        lines_X = f.readlines()
...
def get_der_fast(S, B, S_mat, gene_list, prediction, bulk_mode = False, likelihood_vars = None):
    ...
    grad = -d1_vec @ S
    hess = (-d2_vec.to_numpy()[:,np.newaxis,np.newaxis] * S_mat).sum(0)
...
def create_RCTD(spatialRNA, reference, max_cores = 4, test_mode = False, gene_cutoff = 0.000125, fc_cutoff = 0.5, gene_cutoff_reg = 0.0002, fc_cutoff_reg = 0.75, UMI_min = 100, UMI_max = 20000000, UMI_min_sigma = 100, MIN_OBS = 3,
                         class_df = None, CELL_MIN_INSTANCE = 25, cell_type_names = None, MAX_MULTI_TYPES = 4, keep_reference = False, cell_type_info = None, loggings = logger):
