"""
LLM Backend for Spatial Transcriptomics Agent

This module provides a lightweight, provider-agnostic backend to call LLMs
directly. It is intentionally minimal: it sends a system prompt plus the user
prompt to the selected model and returns the text content of the response.
...
environment, or passed explicitly to the backend constructor.
"""
...
OPENAI_COMPAT_BASE_URLS = {
    ...
    "poe": "https://api.poe.com/v1",  # Poe API (OpenAI-compatible)
...
class LLMBackend:
    """Simple, async LLM client used by the Spatial Transcriptomics Agent.
    ...
    async def stream(self, user_prompt: str):
        ...
        Examples
        --------
        >>> backend = LLMBackend(system_prompt="...", model="gpt-4o")
